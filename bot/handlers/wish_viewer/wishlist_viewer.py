...
async def edit_user_wishlist_editor(bot: AsyncTeleBot, logger: Logger, message: Message, target_user_id: int,
                                    wish_manager: WishManager,
                                    page_idx: int) -> None:
    request = await _make_editor_config(logger, wish_manager, message.from_user.id, target_user_id, page_idx)
    if request is None:
        await bot.send_message(message.chat.id, 'Что-то пошло не так, не мог бы ты попробовать снова?')
        return
