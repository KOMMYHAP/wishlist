...
class WishStorageMemoryAdapter(WishStorageBaseAdapter):
    ...
    def _get_user(self, user_data: dict) -> User | None:
        try:
            raw_update_time = user_data.get('wishlist_update_time', 0)
            wishlist_update_time = datetime.datetime.fromtimestamp(raw_update_time, datetime.UTC)
            return User(
